...
//     console.log('1. Raw settings:', rawSettings);
//     console.log('2. System settings exists?', !!rawSettings?.systemSettings);
//     console.log('3. Landing data exists?', !!rawSettings?.systemSettings?.landing);
    
//     if (rawSettings?.systemSettings?.landing) {
//       const landingData = rawSettings.systemSettings.landing;
//       console.log('4. Landing data object:', landingData);
//       console.log('5. Hero title (EN):', landingData.hero?.title);
//       console.log('6. Hero title (AR):', landingData.hero?.titleAr);
//       console.log('7. Services enabled?', landingData.services?.enabled);
...
//       console.log('10. EXPECTED Hero title: "kikikik"');
//       console.log('11. ACTUAL Hero title:', landingData.hero?.title);
//       console.log('12. MATCHES?', landingData.hero?.title === 'kikikik');
//     } else {
//       console.log('13. ERROR: No landing data found in systemSettings!');
//       console.log('14. systemSettings keys:', rawSettings?.systemSettings ? Object.keys(rawSettings.systemSettings) : 'none');
//     }
//   }, [rawSettings]);
...
//   // Get landing settings from systemSettings
//   const landing = rawSettings?.systemSettings?.landing || {};
...
//   const heroEnabled = hero.enabled !== false; // Default to true
...
//   // If admin configured to use selected reviews, pull from redux reviews
//   const reduxReviews = useSelector(s => s.reviews?.list || []);
//   let testimonialItems = testimonials.items || [];
//   if ((testimonials.mode || 'all') === 'all') {
//     // show all approved reviews from backend when admin chooses 'all'
//     testimonialItems = reduxReviews;
//   } else if ((testimonials.mode || '') === 'selected' && (testimonials.featuredIds || []).length) {
//     testimonialItems = (testimonials.featuredIds || []).map(id => reduxReviews.find(r => r._id === id)).filter(Boolean);
//   }

//   // Get enabled services (admin-controlled). No hardcoded fallback items.
//   const enabledServices = (services.items || []).filter(service => service.enabled !== false);
//   const actionCards = enabledServices.map(service => ({
//     image: service.image || null, // Admin controls image URL; do not substitute local defaults
...
//   // Instagram posts must be provided by admin under landing.instagram.posts
//   const instagramPosts = landing.instagram?.posts || [];
...
//   // Image expansion functions (keep as before)
...
//       {/* Debug UI removed — admin-controlled settings only */}
...
//                         <span className="text-lg font-semibold">{expandedImage?.likes}</span>
//                       </div>
...
//           {/* Public page: debug info removed to ensure only admin-controlled content is shown */}
...
//                   {(() => {
//                     const heroImageSrc = hero.image || settings.branding?.logoUrl || null;
...
//                 {isRTL 
//                   ? (hero.subtitleAr || hero.subtitle || hero.descriptionAr || hero.description || settings.description) 
//                   : (hero.subtitle || hero.description || settings.description)}
//               </p>
...
//           {/* SERVICES CARDS - Only if enabled */}
//           {renderSection(servicesEnabled, (
...
//           {renderSection(instagramEnabled, (
//             <div className={`mb-12 transition-all duration-700 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
...
//           {renderSection(testimonialsEnabled, (
//             <div className={`mb-12 transition-all duration-700 ${isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
//               <div className="max-w-4xl mx-auto text-center mb-8">
//                 <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">
...
//                       <p className="text-gray-700 dark:text-gray-200 mb-4">{tst.content}</p>
//                       <div className="flex items-center justify-between">
//                         <span className="font-semibold">{tst.name}</span>
