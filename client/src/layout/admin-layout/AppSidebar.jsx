...
// Import icons - ADD TagIcon to imports
import {
  ChevronDownIcon,
  GridIcon,
  HorizontaLDots,
...
const AppSidebar = () => {
  ...
  const navItems = useMemo(() => [
    ...
    { icon: <ShootingStarIcon />, name: t("admin.offers"), path: "/admin/offers" },
        ...
        <li key={nav.name}>
          {nav.subItems ? (
            <button
              ...
              className={`flex items-center w-full px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 group ${openSubmenu?.type === menuType && openSubmenu?.index === index
            ...
            >
              <span
                className={`flex items-center justify-center w-5 h-5 ${openSubmenu?.type === menuType && openSubmenu?.index === index
            ...
            nav.path && (
              <Link
                ...
                className={`flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 group ${isActive(nav.path)
                    ? "bg-brand-50 text-brand-600 dark:bg-brand-900/20 dark:text-brand-400"
                    : "text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800"
                  } ${!isExpanded && !isHovered
                    ? "lg:justify-center"
              ...
              >
                <span
                  className={`flex items-center justify-center w-5 h-5 ${isActive(nav.path)
                      ? "text-brand-500 dark:text-brand-400"
                      : "text-gray-500 group-hover:text-gray-700 dark:text-gray-400 dark:group-hover:text-gray-300"
                    }`}
                >
  ...
  return (
    ...
    >
      ...
      <div className={`hidden lg:flex items-center px-4 py-4 border-b border-gray-200 dark:border-gray-800 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-between"
        }`}>
            ...
            <>
              <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-gray-100 dark:border-gray-700 shadow-sm flex-shrink-0">
                <img
                  ...
                  alt={settings.restaurantName || t("welcome")}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 lg:py-6">
        <nav className="space-y-6">
          <div>
            <h2
              className={`mb-3 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400 ${!isExpanded && !isHovered ? "lg:hidden" : "px-4"
                }`}
            >
              {isExpanded || isHovered || isMobileOpen ? (
                t("admin.menu_label")
